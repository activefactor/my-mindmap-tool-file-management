...
> v2.0 は、外部レビュー（`docs/基本設計書_Phase2_レビュー報告書_20260725.md`）の
...
| バックエンド | **PHP 8.5（CGI版）に確定**（2026-07-29 契約アカウントで確認済み）。
  `composer.json` の `"php"` 制約は `"~8.5.0"` とする |
| データベース | **MySQL 8.4 に確定**（2026-07-29 契約アカウントで確認済み）。8.0以上のため
  `JSON` 型は問題なくサポートされる（`LONGTEXT` フォールバックは不要）。文字コード・
  照合順序は §5.1 参照 |
...
| 対象ドメイン | `mindmap.activefactor.org`（2026-07-29 確定） |

### 1.3 全体構成（フロント／バックエンドの配置）

> **確認結果（2026-07-29）**: 対象ドメイン `mindmap.activefactor.org` の実際の公開ディレクトリは
> `/web/activefactor.org/mindmap/` であることを契約アカウントで確認済み（サブドメイン
> `mindmap` が、契約ドメイン `activefactor.org` の `web/` 配下のサブフォルダ `mindmap/` に
> マッピングされている）。下記の構成図はこの確定パスを反映済み。

```
[ホームディレクトリ]（Web非公開。/web/ と同階層）
...
│   └── db/migrations/
│
[web/activefactor.org/mindmap/]（Web公開。mindmap.activefactor.org の公開ディレクトリ）
├── index.html, assets/                # フロントエンド ビルド成果物（dist/ をそのまま配置）
├── api/
│   └── index.php                      # フロントコントローラ。ホームディレクトリ直下の
│                                       # mindmap-app/src までは4階層上（web/activefactor.org/
│                                       # mindmap/api/ → ../../../../mindmap-app/src）。
│                                       # `realpath()` で解決した絶対パスが非公開ディレクトリ
│                                       # 配下であることを起動時にアサートする
...
- UserInfo エンドポイントを参照する場合は、取得した `sub` と ID トークンの `sub` が一致する
  ことを必ず確認する（トークン差し替え防止）。
  ただし **検証済みの ID トークンに `email` が含まれている通常のケースでは UserInfo を呼ばない**
  （不要な往復を避け、トークン差し替えの機会自体を作らないため）。`email` が得られない場合
  （Microsoft のアカウント種別によってはあり得る）のみ UserInfo を参照し、上記の `sub` 一致
  確認を行う。
...
- ログイン時の解決ロジック:
  ...
  2. 一致しない場合、許可判定を通過していれば `users.email` を検索する。
     - 一致するユーザーが **存在しない** 場合: 新規に `users` と `user_identities` を作成する
       （初回ログイン＝新規登録）。
     - 一致するユーザーが **存在し、かつそのユーザーに紐付く `user_identities` が
       1件も無い** 場合（例: `db/seed.php` による初期管理者ブートストラップのように、
       事前に `users` 行だけが用意されているケース）: **その未ログインアカウントへの
       初回ログインとして扱い**、新しい `user_identities` を通常どおり追加する
       （＝conflict にしない）。これが無いと、シードで作成した初期管理者が
       永久にログインできなくなる（Step 2 実装時に発見した不具合）。
     - 一致するユーザーが **存在し、かつ既に別プロバイダの `user_identities` が
       1件以上存在する** 場合: **自動では統合しない**。ログインを拒否し、
       「このメールアドレスは別のログイン方法で登録されています。そちらでログインするか、
       ログイン後の設定画面からアカウント連携を行ってください」という趣旨のエラーを
       表示する（監査ログに `login_denied_conflict` を記録）。
...
- 自分自身のロールを一般ユーザーに変更する操作は禁止する。**自分自身の無効化も同様に
  禁止する**（2026-08-01 追記）。自己無効化は「自分を締め出す」という点で自己降格と同種の
  事故であり、無効化された本人には元に戻す手段が無い（管理APIは `status = 'active'` を
  要求するため）。実装では方向によらず「実行者 == 対象」を一律で拒否する。
- **最後の管理者保護（重大な修正: 競合条件対策）**: 管理者を無効化・降格する操作は、
  ...
  > **実装時の補足（2026-08-01）**: 「残る管理者の確認」も `FOR UPDATE` で行う。これにより、
  > 2人の管理者が同時に互いを降格しようとすると双方が相手の行のロックを待ち、MySQL が
  > デッドロック（1213）またはロック待ちタイムアウト（1205）としてどちらか一方を中断する。
  > アプリはこれを `409 concurrent_modification` に変換して返す。
  >
  > なお、**逐次実行の経路ではこのチェックは発火しない**。管理APIは実行者が有効な管理者で
  > あることを要求し、かつ自分自身は変更できないため、対象を除いても実行者自身が常に
  > 「残る有効な管理者」として存在するためである。したがってこの保護は
  > **同時実行に対する防御と多層防御として機能する**（将来、自己降格を許す・バッチ処理から
  > 呼ぶといった変更が入った場合に効く）。
...
- 補助的に `Origin` ヘッダーを検証し（自ドメインのみ許可）、`Origin` が送信されない場合に
  限り `Referer` を確認する。
- JSON API は `Content-Type: application/json` を必須とする（実装は Step 4。ボディを伴う
  リクエストのみを対象とし、`DELETE` のようにボディを持たないリクエストは対象外とする。
  違反時は `415` を返す）（フォーム経由の単純な CSRF 攻撃を
  形式面でも防ぐ）。
...
> **Composer の導入について（確認結果）**: heteml は SSH 接続を全ユーザーに無料で提供しているが、
> Composer はサーバーにプリインストールされていない。SSH でログインした後、
> `curl -sS https://getcomposer.org/installer | php` 等でユーザーのホームディレクトリ配下に
> Composer 自体を手動インストールし、以後 `php composer.phar install` の形で利用する必要がある
> （`composer` コマンドとして直接使いたい場合は `PATH` の設定も必要）。デプロイ手順書
> （Step 11）にこの手順を明記する。
...
| 2 | ~~ルーティングライブラリの確定~~ | **解消（2026-07-31）**: サーバー側は自前の軽量ルーター（`src/Http/Router.php`）を採用。`{name}` を1セグメントのプレースホルダとして扱うのみで十分であり、依存を増やさない方針（§9）に沿う |
| 3 | ~~フロントエンドルーティングライブラリ~~ | **解消（2026-08-01）**: `react-router-dom` v7 を採用。サーバー側の認証フローが `/dashboard` `/login?error=...` へのパスベースのリダイレクトを前提に実装済みで、URLと画面の対応が必須要件になっているため。選定理由は ADR `mindmap-tool/docs/adr/20260801_フロントエンドルーティングライブラリ選定.md` に記録 |
| 4 | ~~heteml の対象ドメインの公開フォルダ名~~ | **解消（2026-07-29）**: `mindmap.activefactor.org` → `/web/activefactor.org/mindmap/` に確定（1.3参照） |
| 5 | ~~契約中のPHP/MySQLバージョン~~ | **解消（2026-07-29）**: PHP 8.5（CGI版）・MySQL 8.4 に確定（1.2参照）。有効拡張モジュール（`zip`, `curl`, `openssl`, `pdo_mysql`, `mbstring`）の実機確認は Step 1（Docker環境構築）で、選定したPHPバージョンのイメージに対して行う |
| 6 | ~~heteml cron の利用可否~~ | **解消（2026-07-29）**: cron は利用可能。ただしレンタルサーバーの共有リソースであるため、高頻度（毎分実行等）のジョブは制限される（[heteml cron設定ヘルプ](https://support.heteml.jp/hc/ja/articles/360042620493-cron-%E8%A8%AD%E5%AE%9A)）。本設計で必要なジョブは Zipクリーンアップ（1時間ごと）とゴミ箱の自動完全削除（1日1回程度）のみで、いずれも低頻度のため制限の影響を受けない見込み。実際の設定可能件数・最小実行間隔は Step 11（本番デプロイ設定）でコントロールパネルの「cron設定」画面から具体的なcron式を登録する際に最終確認する |
...
| 10 | 「利用リージョンを日本のみに限定」の担保手段の追加検討 | 現在は許可ドメイン／許可アドレス制御のみで、アクセス元地域そのものは保証しない。heteml の「海外アタックガード」機能を調査したところ、**WordPress/MovableType等の管理画面など、あらかじめ攻撃対象になりやすいと定義された既知のディレクトリ・ファイル名のみを保護対象とする機能**であることが公式情報から判明した。本アプリのような独自実装のログイン／API経路（`/api/auth/*` 等）がこの保護対象に含まれるかどうかは公開情報からは確認できず、対象外である可能性が高い。**今回の設計・要件確認の範囲では対応せず、実際に有効化するか・独自パスに適用できるかの判断は構築・デプロイ時に heteml サポートへの確認も含めて行う**（要件定義書の文言は現状維持） |
...
| 2.0 | 2026-07-25 | 外部レビュー（`docs/基本設計書_Phase2_レビュー報告書_20260725.md`）の指摘を反映。主な変更: セッションCookieを `SameSite=Strict` から `Lax` に変更しCSRFトークン検証を追加、heteml公開ディレクトリ構成を `public_html/` から `web/<ドメイン>/` 前提に修正、プロバイダアカウントの自動統合を廃止し `provider+sub` を identity の主キーに変更、オートセーブに revision・リクエスト直列化・画面遷移時flushを追加、ユーザー無効化/ロール変更時に `security_stamp` で既存セッションを即時失効、フォルダ移動時の所有者検証とDB複合FKを追加、サーバー側バリデーション/容量制限/レート制限/CSRF/HTTPセキュリティヘッダー/エラーログ取り扱い/監査ログ運用（新設 §6章）を追加、マップ削除をソフトデリート（ゴミ箱・復元・完全削除、FR-14）として設計、マップタイトル/日時の正本をDB側に統一。要件定義書.md v2.1 の FR-14, NFR-S-9/10, NFR-A-6 に対応 |
| 2.1 | 2026-07-29 | 開発ステップ_Phase2.md Step 0（heteml実機確認）の一環として、heteml公式サポート情報を調査。1.3 の `web/<ドメイン>/` 構成を「暫定」から「確認済み」に更新、1.2 のPHP(8.2〜8.5提供)・MySQL(8.0以上でJSON型対応)の情報を具体化、9章にComposerの手動インストール手順が必要である旨を追記、12章の未決定事項を調査結果に基づき更新（特に No.10: 海外アタックガードは既知CMSパスのみが対象で独自アプリには適用されない可能性が高いと判明） |
| 2.2 | 2026-07-31 | ユーザーからの実機情報提供を受け、未決定事項 No.4/5/6 を解消。対象ドメイン `mindmap.activefactor.org`、公開パス `/web/activefactor.org/mindmap/`、PHP 8.5（CGI版）、MySQL 8.4 に確定。1.2/1.3 の構成図・バージョン記載を確定情報に更新し、cronは低頻度ジョブのみのため利用可能と判断 |
| 2.5 | 2026-08-01 | Step 4（管理コンソール）の実装に伴い更新。§3.2 に「自分自身の無効化も禁止する」を追記し、最後の管理者保護が逐次実行では発火せず同時実行への防御として機能することを実装時の補足として明記。§6.1 の Content-Type 必須化の適用範囲（ボディを伴うリクエストのみ・違反時415）を明確化。§12 未決定事項 No.2（サーバー側ルーティングライブラリ = 自前ルーター）と No.3（フロントエンドルーティング = `react-router-dom`）を解消 |
| 2.4 | 2026-07-31 | Step 3（認証基盤）の実装に伴い §3.1 の UserInfo の扱いを明確化: IDトークンに `email` が含まれる通常のケースでは UserInfo を呼ばず、不足する場合のみ参照して `sub` の一致を確認する方式とした（不要な往復とトークン差し替えの機会を減らすため）。また §12 未決定事項 No.1（Microsoft OAuth・IDトークン検証方式）を ADR `server/docs/adr/20260731_oidc実装方式とライブラリ選定.md` により解消 |
| 2.3 | 2026-07-31 | Step 2（DBスキーマ実装）の `db/seed.php` 設計中に発見した不具合を修正: §3.1 のアカウント解決ロジックで、既存ユーザーに `user_identities` が1件も無い場合（初期管理者シード直後の未ログイン状態等）を「初回ログイン」として扱い、conflict にしないよう明確化。修正前の記述では初期管理者が永久にログインできなくなる欠陥があった |
